...
function getListOfMethodData(decoded: Record<string, any>, y_loc_string: string): string[] {
    try {
        ...
        if (levels.length === 1) {
            ...
            if (decoded_item) {
                if (Array.isArray(decoded_item)) {
                    return decoded_item.map((d) => d.toString());
                } else {
                    return [decoded_item.toString()];
                }
            } else {
                return [];
            }
...
//     );
//     const wantedData = getListOfMethodData(decoded, "params->inputs->lootEntity");
