...
export const createChallengeSchema = z.object({
    ...
    forwarderAddress: z.string({ message: Errors.FORWARDER_ADDRESS }).regex(/^(0x)?[0-9a-fA-F]{40}$/, {message: "Forwarder address must be a valid ethereum address"}).optional().nullable(),
    forwarderABI: jsonSchema.optional().nullable(),
    methodDataAttributeName: z.string({ message: Errors.METHOD_DATA_ATTRIBUTE_NAME }).optional().nullable(),
    wantedData: z.string({ message: Errors.WANTED_DATA }).optional().nullable(),
